...
POOL_SIZE = 4  # Maximale Checkpoints im Selbstspiel-Pool
MAX_STEPS_EP = 200
...
class SelfPlayCallback(BaseCallback):
    ...
    def __init__(
        self,
        opponents: list[OpponentBot],
        save_dir: Path,
        save_freq: int = SELFPLAY_FREQ,
        verbose: int = 1,
    ):
...
def main() -> None:
    ...
    opponents = [OpponentBot(SERVER_URL, room=f"training-{i}") for i in range(N_ROOMS)]
    for opp in opponents:
        opp.start()
